...
| `REQ-AUTH-03` | Authentication | User logout & server-side token revocation via Redis | `server/src/routes/auth.ts` |
...
| `REQ-ART-05` | Access Control | Draft visibility isolation & publication workflow | `tests/api/smokeTest.spec.ts` |
| `REQ-CACHE-01` | Redis Caching | Cache MISS on cold request, Cache HIT on repeat | `server/src/routes/articles.ts` |
| `REQ-CACHE-02` | Cache Eviction | Cache invalidation triggered by article mutation | `server/src/redis.ts` |
| `REQ-DB-01` | DB Integrity | Direct SQL query verification on PostgreSQL tables | `server/src/db/` |
...
```bash
# Run API smoke & contract test suites
...
### 1. Swagger UI Exploratory Testing
...
### 2. Live SQL Query & Redis Cache Logs
...
### 3. Database State Verification (psql)
```bash
npm run db:psql
```
```sql
...
### 4. Redis Cache Verification (redis-cli)
```bash
npm run redis:cli
```
```text
